...
try{
	...
	function askForUserName(id){
		...
		//document.querySelectorAll(".modoSound").style.visibility = "visible";
		// setVisibilityUI("span-Sound","visible");
		// setVisibilityUI("modoSound","visible");
		// setVisibilityUI("gameType","visible");
...
const jugar = () => {	
	...
	setVisibilityUI('iconSound-container',"visible");
	
	soundFicha = document.querySelector("#soundFicha");
	document.getElementById("nextPlayer").innerHTML = "Le toca al jugador: " + nextPlayer();
		...
		cols.forEach(element => {							
			...
			cols.forEach(col => {	
				...
				if (!isWinner(lastPossibleRound.substring(0,1),lastPossibleRound.substring(1,2))){
					...
					if (soundIsEnabled()){
					let soundWin = document.querySelector("#soundWin").play();
				}
					document.getElementById("nextPlayer").style.textAlign = "center";
					document.getElementById("nextPlayer").style.fontSize = "34px";
					document.getElementById("nextPlayer").innerHTML = "¡" + jugadorActual + " ha ganado!";
					isGameOver = true;
					clearInterval(arrowBlinkMove);					
					clearTimeout(fArrowBlink);
